...
<body>
    <div class="container">
		...
		<div class="span10 offset1">
			<div class="row">
				<h3>Assign a Driver/Rider to Location</h3>
			</div>
			...
			<form class="form-horizontal" action="fr_assign_create.php" method="post">
				...
				<div class="control-group">
					...
					<div class="controls">
						<?php
							$pdo = Database::connect();
							$sql = 'SELECT * FROM jeepney_persons ORDER BY lname ASC, fname ASC';
				...
				<div class="control-group">
					<label class="control-label">Location Info</label>
					<div class="controls">
						<?php
							$pdo = Database::connect();
							$sql = 'SELECT * FROM fr_fares ORDER BY fare_date ASC, fare_time ASC';
							...
							if($eventid) // if $_GET exists restrict event options to selected event (from $_GET)
								foreach ($pdo->query($sql) as $row) {
									if($eventid==$row['id'])
									echo "<option value='" . $row['id'] . " '> " . Functions::dayMonthDate($row['fare_date']) . " (" . Functions::timeAmPm($row['fare_time']) . ") - " .
									trim($row['fare_description']) . " (" . 
									trim($row['fare_location']) . ") " .
									"</option>";
								}
							else
								foreach ($pdo->query($sql) as $row) {
									echo "<option value='" . $row['id'] . " '> " . Functions::dayMonthDate($row['fare_date']) . " (" . Functions::timeAmPm($row['event_time']) . ") - " .
									trim($row['fare_description']) . " (" . 
									trim($row['fare_location']) . ") " .
									"</option>";
								}
