				...
				<div class="control-group">
					...
					<div class="controls">
						<?php
							...
							foreach ($pdo->query($sql) as $row) {
								if($row['id']==$event) {
									echo "<option selected value='" . $row['id'] . " '> " . Functions::dayMonthDate($row['fare_date']) . " (" . Functions::timeAmPm($row['fare_time']) . ") - " . trim($row['fare_description']) . " (" . trim($row['fare_location']) . ") " . "</option>";
								}
								else {
									echo "<option value='" . $row['id'] . " '> " . Functions::dayMonthDate($row['fare_date']) . " (" . Functions::timeAmPm($row['fare_time']) . ") - " . trim($row['fare_description']) . " (" . trim($row['fare_location']) . ") " . "</option>";
								}
							}
							echo "</select>";
							Database::disconnect();
